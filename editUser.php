    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if(isset($_POST['updateUser'])) {
            ...
            if (empty($name) || 
                empty($email)) {
                // Si les champs sont vides
                header("Location: editpage.php?id=$id&error=champsvides"); 
                exit();

            } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) { // si l'email est invalide
                header("Location: editpage.php?id=$id&error=emailinvalide");
                exit();

            } elseif ($nbDoublons > 0) {
                // Redirection avec message d'erreur si email déjà utilisé
                header("Location: editpage.php?id=$id&error=emailpris");
                exit();
            }
            else {
                ...
                header("Location: listUsers.php?success=edited");
                exit();
            }
        }
    }
